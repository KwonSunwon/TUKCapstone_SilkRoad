...
void FBXLoader::LoadMeshWithAssimpNotAnim(FbxMesh* mesh, aiMesh* assimpMesh)
{
	...
	const int32 triCountByAssimp = assimpMesh->mNumFaces; // �޽��� �ﰢ�� ������ �����´�
	assert(triCount == triCountByAssimp);
	for (int32 i = 0; i < triCountByAssimp; i++) // �ﰢ���� ����
	{
...
void FBXLoader::LoadBoneWeight(aiBone* bone, int32 boneIdx, FbxMeshInfo* meshInfo)
{
	...
	{
		aiVertexWeight& weight = bone->mWeights[i]; // 가중치 정보 가져오기
		int32 vtxIdx = weight.mVertexId; // 정점 인덱스 가져오기
		float w = weight.mWeight; // 가중치 값 가져오기
		meshInfo->boneWeights[vtxIdx].AddWeights(boneIdx, w); // 가중치 추가
	}
}
